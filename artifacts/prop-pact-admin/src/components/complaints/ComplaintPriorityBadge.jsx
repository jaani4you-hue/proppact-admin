...
export default function ComplaintPriorityBadge({ priority }) {
  ...
  return (
    <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-[11px] font-semibold ${cls}`}>
      {priority === 'Critical' && <span className="mr-1.5 h-1.5 w-1.5 rounded-full bg-red-500 animate-pulse" />}
      {priority !== 'Critical' && <span className="mr-1.5 h-1.5 w-1.5 rounded-full bg-current opacity-80" />}
      {priority ?? 'Unknown'}
    </span>
  );
}
