...
export function VendorSkeletonTable({ rows = 5 }) {
  return (
    <div className="overflow-x-auto rounded-xl border border-gray-100 bg-white shadow-sm">
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-gray-100 bg-gray-50/60">
            {['Vendor', 'Category', 'Contact', 'Status', 'Rating', ''].map((h) => (
        ...
        <tbody>
          {Array.from({ length: rows }).map((_, i) => (
            <tr key={i} className="border-b border-gray-50">
              ...
              <td className="py-3.5 px-4 space-y-1.5"><Skel w="w-28" /><Skel w="w-36" h="h-3" /></td>
              <td className="py-3.5 px-4"><Skel w="w-16" /></td>
              <td className="py-3.5 px-4"><Skel w="w-12" /></td>
              <td className="py-3.5 px-4"><Skel w="w-16" /></td>
            </tr>
          ))}
        </tbody>
